...
pcb* Find_PCB_Ready(char* name) {
    ...
    while(walk != NULL) {
      ...
      if (walk== NULL){
      		printf("The pcb %s you are looking for is not in the ready queue.", name);
      }
...
pcb* Find_PCB_Blocked(char* name) {
    ...
    while(walk != NULL) {
      ...
      if (walk== NULL){
      		printf("The pcb %s you are looking for is not in the blocked queue.", name);
      }
}
...
pcb* Find_PCB_Suspended_Ready(char* name) {
    ...
    while(walk != NULL) {
      ...
      if (walk== NULL){
      		printf("The pcb %s you are looking for is not in the suspended ready queue.", name);
      }
}
...
pcb* Find_PCB_Suspended_Blocked(char* name) {
    ...
    while(walk != NULL) {
      ...
      if (walk== NULL){
      		printf("The pcb %s you are looking for is not in the suspended blocked queue.", name);
      }
}
...
void Show_PCB(char* name) {
  ...
  if (FALSE) {
    ...
    if (pcbPtr2->state == RUNNING) {
      ...
      temp = "SUSPENDED_READY";
    }else if (pcbPtr2->state == SUSPENDED_BLOCKED) {
      temp = "SUSPENDED_BLOCKED";
    }
    else {
      temp = "INVALID STATE";
    }
